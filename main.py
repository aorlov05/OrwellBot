...
async def judge(ctx, *, string: str = ""):
    ...
    punishment = genai_client.models.generate_content(model="gemini-2.0-flash-lite", contents=prompt + "User, " + str(
        ctx.author) + " says:" + string + "Respond only with 'kick', 'ban', or 'timeout', followed by a colon ':' and a brief reason for the punishment in under 100 characters.")
